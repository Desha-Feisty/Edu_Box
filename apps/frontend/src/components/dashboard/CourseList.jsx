...
export default function CourseList({ courses = [], onSelect }) {
  ...
  return (
    <div className="space-y-2">
      {courses.map((c) => (
        <button key={c.id} onClick={() => onSelect && onSelect(c)} className="w-full text-left bg-white dark:bg-base-200 p-3 rounded-lg border border-slate-200/60 dark:border-white/[0.04] hover:shadow">
